...
## 13. `b45c26a` - BlockCache / CacheChain 等待链

- 时间：2026-06-24 21:17:47 +0800
- 范围：`kernel/src/kernel.rs`、`chaos-tests/tests/custom/`
- 提交信息：`fix BlockCache 可能的死锁`
- 统计：73 行新增，13 行删除；代码侧主要修改 `BlockCache::fetch`
- 相关远程测试：`group_01::adv_scheduler_fs_memory_deadlock_chain`

主要发现：

- 新一轮远程日志显示 `id=2010` 在等待 GKL，当前 owner 是 `2020`；另一个线程随后以 `current_is_holder=false` 调用 `GKL.leave()`。这更像是测试超时后的清理逻辑在强行释放 GKL，而不是根因。
- 真正可疑链路是 `BlockCache::fetch`、`CacheChain.lk` 和 `Kernel::tick`：`fetch(k, lat)` 在 cache miss 时持有 `CacheChain.lk` 执行 `thread::sleep(lat)`，模拟慢 I/O；而 `tick(id)` 会先进入 GKL，再遍历所有 cache chain 并等待每个 `ch.lk`。
- 因此可能形成等待链：线程 A 在 `fetch` 里持有某个 cache chain 锁睡眠；线程 B 执行 `tick(2020)`，持有 GKL 后等待该 chain 锁；线程 C 再进入 GKL 时只能等待 owner `2020`，最终表现为高级测试超时。

主要改动：

- 将 `BlockCache::fetch` 的慢路径改为：首次查 cache miss 后先释放 `CacheChain.lk`，再执行 `sleep(lat)` 模拟 I/O，避免在自旋锁保护区内阻塞。
- 在 `fetch` 重新拿到 `CacheChain.lk` 后增加二次查找，防止两个线程同时 miss 同一个 key 时重复插入相同 `CacheSlot.id`。
- 新增 `chaos-tests/tests/custom/cache_chain_regression.rs`，用 `run_with_timeout` 风格构造慢 `fetch` 与 `tick(2020)` 并发的场景，验证 `tick` 不应因为慢 cache miss 而长期持有 GKL。

验证结果：

- `cargo test --test custom -- --test-threads=1 --nocapture`：3/3 通过。
- `cargo test --test basic -- --test-threads=1`：33/33 通过。

整体理解：GKL 本身这轮日志没有直接暴露新的重入错误；更核心的问题是 cache miss 慢路径在持有 `CacheChain.lk` 时睡眠，间接让持有 GKL 的 `tick` 被拖住。修复重点应放在缩短 cache chain 自旋锁临界区，而不是让 GKL 或 `tick` 去承担慢 I/O 等待。
